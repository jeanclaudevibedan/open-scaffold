...
- **Plans directory:** `.scaffold/plans/` — immutable plan files, one per task/feature slice, conforming to the 7-section schema in `.scaffold/plans/handoff-template.md`.
...
2. **Every non-trivial change must trace to a plan file** in `.scaffold/plans/` that follows the handoff template schema.
