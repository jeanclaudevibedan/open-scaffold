...
Plans in `.scaffold/plans/` are immutable; scope evolution is captured via `<slug>-amendment-<n>.md` files plus a one-line entry in MISSION.md's Changelog. The mechanical steps (find next N, scaffold the file, append the changelog line) are currently manual. The amendment schema exists at `.scaffold/plans/README.md:7-13` (Parent / Date / Learning / New direction / Impact on acceptance criteria) but has never been exercised — no amendment file has been written against any plan in this repo. `verify.sh` already enforces sequential numbering (Check 3) and changelog containment of amendment basenames (Check 4), so any helper that passes those checks is automatically correct. This plan adds a fourth bash script, `amend.sh`, that operationalizes the existing schema, and a short paragraph in CLAUDE.md / AGENTS.md telling agents to drive it conversationally.

## Goal

A user can run `./amend.sh <plan-slug>` in a fresh open-scaffold checkout and end up with a correctly numbered, schema-complete amendment file in `.scaffold/plans/` and a matching one-line entry in MISSION.md's Changelog — with zero manual bookkeeping, and with `./verify.sh --standard` passing afterward.
...
- No new schema invention. The 5-section schema already defined in `.scaffold/plans/README.md` is the authoritative source; the script scaffolds it verbatim. If that schema is wrong, that is a separate change to README.md, not to this script.
...
- Does not ship as a `.scaffold/` skill or Claude command — it is a first-class repo script alongside the other three.
...
- `.scaffold/plans/README.md` — add a one-line pointer at the bottom: "Tip: run `./amend.sh <plan-slug>` from the repo root to scaffold the next amendment and stamp the changelog automatically."
- `README.md` — optional one-line mention in the scripts list if one exists (verify before editing; do not invent a section).

Does NOT touch: `MISSION.md` (only modified at runtime by the script, never at plan-authoring time), any file under `.scaffold/plans/` other than `README.md`.
...
- `--message "<text>"` (optional) — override the default changelog entry text. If omitted, the entry reads: `YYYY-MM-DD: amendment <N> to <plan-slug> — see .scaffold/plans/<plan-slug>-amendment-<N>.md`.

**Preconditions**

1. Must be run from or resolve to the repo root containing `MISSION.md` and `.scaffold/plans/`. Use the same `ROOT="$(cd "$(dirname "$0")" && pwd)"` pattern used by the other three scripts.
2. Parent plan file `.scaffold/plans/<plan-slug>.md` must exist. If it does not, exit 1 with: `Error: parent plan .scaffold/plans/<plan-slug>.md not found. Amendments must target an existing plan.`
...
1. Scan `.scaffold/plans/<plan-slug>-amendment-*.md`. For each match, strip the prefix and suffix to get the integer, track the max.
2. Next amendment number N = max + 1. If no existing amendments, N = 1.
3. If `.scaffold/plans/<plan-slug>-amendment-<N>.md` already exists (race or filesystem oddity), exit 1 with a clear error; never overwrite.

**Scaffold content**

Write `.scaffold/plans/<plan-slug>-amendment-<N>.md` with exactly the 5-section schema from `.scaffold/plans/README.md:7-13`:
...
1. Build the changelog line: `- <DATE>: amendment <N> to <plan-slug> — see .scaffold/plans/<plan-slug>-amendment-<N>.md` (or the custom message if `--message` was passed, with the amendment filename appended so `verify.sh` Check 4 still finds the basename).
...
```
Created: .scaffold/plans/<plan-slug>-amendment-<N>.md
...
- [ ] Running `./amend.sh amend-helper` against a fresh checkout produces `.scaffold/plans/amend-helper-amendment-1.md` containing the exact 5 sections listed in the scaffold spec above, with `date +%Y-%m-%d` in the Date field.
...
- [ ] Running `amend.sh` does NOT modify `.scaffold/plans/amend-helper.md` (plan immutability preserved; check with `git diff` on the parent plan file).
...
- [ ] `.scaffold/plans/README.md` gets the one-line "Tip:" pointer at the bottom, and no other changes.
...
3. Run `./amend.sh amend-helper` a second time → check `-amendment-2.md` exists, `-amendment-1.md` is unchanged (`git diff --exit-code .scaffold/plans/amend-helper-amendment-1.md`).
...
8. Destroy `.scaffold/plans/amend-helper-amendment-1.md`, run `./amend.sh amend-helper` → numbering should regenerate as `-amendment-1.md` (numbering is based on current filesystem state, which is the correct behavior for a no-agent tool — the agent/user is responsible for not deleting committed amendments).
...
1. **Operationalize, don't re-invent.** The amendment schema is already written in `.scaffold/plans/README.md:7-13`. This helper scaffolds from the existing schema; it does not mint a new one.
...
**Option C: Pure documentation — expand `.scaffold/plans/README.md` to include a worked example and call it done.**
...
- The amendment schema already exists at `.scaffold/plans/README.md:7-13` but has never been exercised (Driver 1).
...
**Follow-ups.**
- After the first real amendment is written through `amend.sh`, decide whether the 5-section schema in `.scaffold/plans/README.md` needs revision (e.g., an optional "Related ADR" field). If yes, revise the schema and the script scaffold in lockstep.
