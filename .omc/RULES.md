...
4. **Folder = status.** Plans live in `active/`, `backlog/`, `done/`, or `blocked/`. Move files, don't rename them. See `.omc/plans/WORKFLOW.md`.
5. **Verify before claiming done.** Run `./verify.sh` against acceptance criteria. Use `./close.sh` to move plans to `done/`.
6. **Check active/ first.** Before starting new work, check `.omc/plans/active/`. Continue in-flight work unless told otherwise.
...
- All plans follow the 7-section schema in `.omc/plans/handoff-template.md`

## When In Doubt

- Structure questions → re-read this file and `.omc/plans/WORKFLOW.md`
